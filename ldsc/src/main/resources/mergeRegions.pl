...
my $s3in=$ENV{"INPUT_PATH"};
my $s3out=$ENV{"OUTPUT_PATH"};
...
`hadoop fs -getmerge -nl -skip-empty-file "$s3in/out/ldsc/regions/$partition_in" "$tmpFile"`;
...
`aws s3 cp "$bedFile" "${s3out}/out/ldsc/regions/$partition_out/$bedFile"`;
