...
def process_gene_datasets(spark):
    ...
    """
    df = spark.read.json(f's3://{s3_in}/gene_associations/combined/*/part-*')

    df = df.withColumn('pValue', when(df.pValue == 0.0, np.nextafter(0, 1)).otherwise(df.pValue))
    genes = spark.read.json(f's3://{s3_in}/genes/GRCh37/part-*')
...
def process_600trait_datasets(spark):
    ...
    phenotype, so they may be queried either way.
    """
    df = spark.read.json(f's3://{s3_in}/gene_associations/600k_600traits/*/*/part-*')
    ...
    df.orderBy(['ancestry', 'gene', 'pValue']) \
        .write \
        .mode('overwrite') \
        .json(f's3://{s3_out}/gene_associations/600trait')
    ...
    df.drop('masks') \
        ...
        .write \
        .mode('overwrite') \
        .json(f's3://{s3_out}/finder/600trait')
...
def process_transcript_datasets(spark):
    ...
    phenotype, so they may be queried either way.
    """
    df = spark.read.json(f's3://{s3_out}/transcript_associations/*/*/part-*')

    # sort by gene, then by p-value
    df.orderBy(['transcript', 'pValue']) \
        .write \
        .mode('overwrite') \
        .json(f'{s3_out}/transcript_associations/55k')
...
def process_magma(spark):
    ...
    # change back
    genes = spark.read.json(f's3://dig-analysis-data/genes/GRCh37/part-*')
