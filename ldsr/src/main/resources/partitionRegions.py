...
def main():
    ...
    outdir = f'{S3DIR}/out/ldsr/regions/partitioned/{args.dataset}'

    # create a spark session
    spark = SparkSession.builder.appName('ldsr').getOrCreate()

    # read all the fields needed across the regions for the dataset
    df = spark.read.json(srcdir)

    # fix whitespace issues
    category = regexp_replace(df.category, ' ', '_')
    annotation = regexp_replace(df.annotation, ' ', '_')
    tissue = regexp_replace(df.tissue, ' ', '_')

    # build the partition name
    partition = concat_ws('___', category, annotation, tissue)
