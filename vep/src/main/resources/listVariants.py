...
def main():
    ...
    variant_counts_srcdir = f'{S3DIR}/variant_counts/*/*/*'
    ...
    variant_counts_df = get_df(spark, variant_counts_srcdir)

    dataset_ld_df = dataset_df.union(ld_server_df)\
        .dropDuplicates(['varId'])
    df = dataset_ld_df.union(variant_counts_df)\
        .dropDuplicates(['varId'])
