...
class CommonVepStage(implicit context: Context) extends Stage {
  ...
  override def make(output: String): Job = {
    ...
    val scripts: Seq[Job.Script] = parts.map { part =>
      Job.Script(resourceUri("runCommonVep.sh"), part, output)
    }
  ...
  override def prepareJob(output: String): Unit = {
    context.s3.rm(s"out/varianteffect/$output/common-effects/")
    context.s3.rm(s"out/varianteffect/$output/common-warnings/")
  }

  /** On success, write the _SUCCESS file in the output directory.
   */
  override def success(output: String): Unit = {
    context.s3.touch(s"out/varianteffect/$output/common-effects/_SUCCESS")
    ()
  }
}
