...
def download_phecode_files(ancestry, cohort, phecode):
    ...
    minp_file = f'{phecode}.minp_formatted.tsv.gz'
    subprocess.check_call(['aws', 's3', 'cp', f'{base_url}/{ancestry}/{cohort}/all/{all_file}', all_file])
    subprocess.check_call(['aws', 's3', 'cp', f'{base_url}/{ancestry}/{cohort}/minp/{minp_file}', minp_file])
    return all_file, minp_file
...
def get_lambda(output, key):
    ...
    non_null_stats = [v for v in stats if v is not None]
    if len(non_null_stats) > 0:
        return np.percentile(non_null_stats, 95) / expected_95th_percentile
...
def get_converted_phenotype_minp(ancestry, cohort, minp_file):
    minp_output = {}
    with gzip.open(minp_file, 'r') as f:
        header = f.readline().decode().strip()
        line = f.readline().decode().strip()
        while len(line) > 0:
            line_dict = dict(zip(header.split('\t'), line.split('\t')))
            if line_dict['most_sig_beta'] != 'nan':
                minp_output[line_dict['gene']] = {
                    'dataset': '440K_MaskPaper',
                    'ancestry': ancestry,
                    'cohort': cohort,
                    ...
                    'gene': line_dict['gene'],
                    'pValue_rare': optional_float(line_dict, 'minP'),
                    'pValue_low_freq':  optional_float(line_dict, 'minP_v2'),
...
def get_full_phenotype_output(all_file, minp_output):
    with gzip.open(all_file, 'r') as f:
        ...
        while len(line) > 0:
            ...
            minp_output[line_dict['gene']]['masks'].append(mask)
            line = f.readline().decode().strip()
    return minp_output
...
def upload_output(ancestry, cohort, phecode, output):
    ...
    with open(f'{phecode}/metadata', 'w') as f:
        f.write(f'{{"name": "440K_MaskPaper", "ancestry": "{ancestry}", "phenotype": "{phecode}"}}\n')
    outdir = f'{s3_out}/gene_associations/440K_MaskPaper/{ancestry}/{cohort}/{phecode}'
