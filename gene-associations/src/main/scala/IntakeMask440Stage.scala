...
class IntakeMask440Stage(implicit context: Context) extends Stage {
  ...
  override val rules: PartialFunction[Input, Outputs] = {
    case mask440(ancestry, cohort, filename) if filename.nonEmpty => Outputs.Named(s"$ancestry/$cohort/$filename")
    case _ => Outputs.Null
  }
  ...
  override def make(output: String): Job = {
    ...
    new Job(Job.Script(
      resourceUri("440KMaskPaperIntake.py"),
      s"--ancestry=$ancestry",
      s"--cohort=$cohort",
      s"--filename=$filename"
    ))
