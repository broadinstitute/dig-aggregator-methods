...
def main():
    ...
    genes = spark.read.json(genes_glob).select("name", "source")
    genes_symbols = genes.filter(genes.source == "symbol").select("name").withColumnRenamed("name", "symbol")
    inspect_df(genes_symbols, "Symbols from genes data")
    nearest_symbols = \
        spark.read.json(variant_effects_glob).select("nearest").withColumn("symbol", col("nearest").getItem(0)) \
            .distinct()
    inspect_df(nearest_symbols, "Nearest from effects")
    symbols = genes_symbols.union(nearest_symbols).distinct()
    inspect_df(symbols, "all symbols we need")
