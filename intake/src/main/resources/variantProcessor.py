...
class IntakeUtilities:
    ...
    def from_metadata_file(data_path, db):
        ...
        metadata['dichotomous'] = metadata['is_dichotomous'] if 'is_dichotomous' in metadata else db.get_is_dichotomous(metadata['phenotype'])
        dataset_data = metadata['dataset_data'] if 'dataset_data' in metadata else db.get_dataset_data(metadata['dataset'])
