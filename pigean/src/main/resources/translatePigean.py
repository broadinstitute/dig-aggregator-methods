...
def download_data(phenotype, file_name, sigma, gene_set_size):
    file_path = f'{s3_in}/out/pigean/staging/pigean/{phenotype}/sigma={sigma}/size={gene_set_size}/{file_name}'
...
def translate_gs(json_line, phenotype, sigma, gene_set_size):
...
def translate_gss(json_line, phenotype, sigma, gene_set_size):
...
def translate_ggss(json_line, phenotype, sigma, gene_set_size):
    beta = make_option(json_line["beta"])
    combined = make_option(json_line["combined"])
...
def main():
    ...
    translate(args.phenotype, args.sigma, args.gene_set_size, 'gene_stats', 'gs.out', translate_gs)
    translate(args.phenotype, args.sigma, args.gene_set_size, 'gene_set_stats', 'gss.out', translate_gss)
    translate(args.phenotype, args.sigma, args.gene_set_size, 'gene_gene_set_stats', 'ggss.out', translate_ggss)
