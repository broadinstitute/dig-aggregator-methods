...
class FactorBetasStage(implicit context: Context) extends Stage {
  ...
  override val rules: PartialFunction[Input, Outputs] = {
    case pigean(traitGroup, phenotype, geneSetSize) => Outputs.Named(s"$traitGroup/$phenotype/$geneSetSize")
  }
  ...
  override def make(output: String): Job = {
    ...
    new Job(Job.Script(resourceUri("factorBetas.py"), flags: _*))
  }
}
