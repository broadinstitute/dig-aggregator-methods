package org.broadinstitute.dig.aggregator.methods.prs
...
class MakePRS(implicit context: Context) extends Stage {
  ...
  override def cluster: ClusterDef = super.cluster.copy(
    ...
    bootstrapScripts = Seq(new BootstrapScript(resourceUri("install-prs.sh")))
  )

  override def make(output: String): Job = {
    val input = MakePRSInput.fromString(output)
    new Job(Job.Script(resourceUri("makePRS.py"), input.flags:_*))
  }

}
  
case class MakePRSInput(
  phenotype: String,
  ancestry: String
) {
...
object MakePRSInput {
  def fromString(output: String): MakePRSInput = {
    output.split("/").toSeq match {
      case Seq(phenotype, ancestry) => MakePRSInput(phenotype, ancestry)
    }
  }
}
