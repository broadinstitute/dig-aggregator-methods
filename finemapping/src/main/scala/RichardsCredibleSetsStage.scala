...
/** final stage to collate all phenotype/ancestry results into single directory
  */
class RichardsCredibleSetsStage(implicit context: Context) extends Stage {
  ...
  override val rules: PartialFunction[Input, Outputs] = {
    case cojo(_) => Outputs.Named("RichardsCredibleSets")
  }
  ...
  override def make(output: String): Job = {
    val script    = resourceUri("richardsCredibleSets.py")
