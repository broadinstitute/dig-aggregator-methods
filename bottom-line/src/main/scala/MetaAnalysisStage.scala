...
class MetaAnalysisStage(implicit context: Context) extends Stage {
  ...
  val variants: Input.Source = Input.Source.Success("bottom-line-with-largest/out/metaanalysis/variants/*/")
...
//      Job.PySpark(loadAnalysis, "--trans-ethnic", phenotype)
  ...
  override def prepareJob(output: String): Unit = {
    context.s3.rm(s"bottom-line-with-largest/out/metaanalysis/staging/ancestry-specific/$output/")
