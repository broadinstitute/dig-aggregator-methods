...
meta_types = ['bottom-line', 'largest', 'random-effects']
...
def get_all_variants(phenotype, ancestry):
    for meta_type in meta_types:
        ...
        if ancestry == 'TE':
            path_to_variants = f'staging/clumped-all/analysis/{phenotype}/variants.json'
        else:
...
def output_and_upload(freq_type, meta_type, phenotype, ancestry, variants_dict, var_id_to_clump, clump_to_metas, overview):
    if ancestry == 'TE':
        path_out = f'{s3_out}/out/metaanalysis/{meta_type}/staging/merged-mrmega/analysis/{phenotype}/'
    else:
